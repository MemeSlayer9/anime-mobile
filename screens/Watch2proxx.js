...
const handleNextEpisode = () => {
  ...
  if (currentIndex < episodes3.length - 1) {
    ...
    if (nextEpisode) {
      // Update the selected episode and series ID
      const selectedEpisodeId = nextEpisode.episodeId;  // Get the next episode's ID
...
// Load qualities when component mounts
  ...
  if (status.isLoaded) {
    ...
    if (data && data.tracks && data.tracks.length > 0) {
      ...
      if (subtitleTrack && subtitleTrack.subtitles) {
        ...
        console.log('No subtitles available for the selected track');
      }
    }
  }
};
